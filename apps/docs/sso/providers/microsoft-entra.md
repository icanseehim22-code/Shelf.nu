...
EstoqueSoftSystem supports single sign-on (SSO) using Microsoft Entra.
...
You'll be using the custom enterprise application setup for EstoqueSoftSystem.

![step-2](../../img/microsoft-entra-step-2.png)

## Step 3: Fill out app details [#](#step-3-fill-out-app-details)

In the modal titled Create your own application, enter a display name for EstoqueSoftSystem. This is the name your Entra users see when signing in to EstoqueSoftSystem using SSO via Entra. We recommend to use the name EstoqueSoftSystem.
...
Before you get to assigning users and groups, which would allow accounts in Microsoft Entra to access EstoqueSoftSystem, you need to configure the SAML details that allows EstoqueSoftSystem to accept sign in requests from Microsoft Entra.

![step-4](../../img/microsoft-entra-step-4.png)

## Step 5: Select SAML single sign-on method [#](#step-5-select-saml-single-sign-on-method)

EstoqueSoftSystem only supports the SAML 2.0 protocol for Single Sign-On, which is an industry standard.

![step-5](../../img/microsoft-entra-step-5.png)

## Step 6: Setup Basic SAML Configuration [#](#step-5-setup-basic-saml-configuration)

In order for the connection to work, you need to make sure your Enterprise application connects to EstoqueSoftSystem.
...
Attribute mappings allow EstoqueSoftSystem to get information about your Microsoft Entra users on each login.
...
## Step 8: Obtain metadata URL and send to EstoqueSoftSystem [#](#step-8-obtain-metadata-url-and-send-to-shelf)
...
It's very important to send this information to your support contact at EstoqueSoftSystem to complete the SSO setup process. If you're not sure where to send this file, you can always reach us at [hello@shelf.nu](mailto:hello@shelf.nu).
...
Once you’ve configured the Microsoft Entra app as shown above, make sure you send the required information to your support contact at EstoqueSoftSystem.
...
### Step 10.2: Give access of the groups to the EstoqueSoftSystem application

Navigate back to your EstoqueSoftSystem enterprise application and go to _Users and Groups_.
...
## Step 11: Map Microsoft Entra groups inside EstoqueSoftSystem [#](#step-9-map-google-workspace-groups-inside-shelf)

> [!NOTE]
> You can only complete this step, once you have received confirmation from your contact person at EstoqueSoftSystem that the setup has been completed.
>
...
Once you have the groups ready, you need to add their IDs in the workspace settings inside EstoqueSoftSystem. If you have multiple workspaces, you will need to map each one.
